...
export function CompanyView({ companyStats, itemsByCompany }: Props) {
  const { activeCompany, setActiveCompany } = useRoadmapStore();

  // Auto-select the first company if none is selected
  useEffect(() => {
    if (!activeCompany && companyStats.length > 0) {
      setActiveCompany(companyStats[0].name);
    }
  ...
  if (companyStats.length === 0) {
    return (
      <div className="py-8 text-center text-[13px] italic text-text-faint">
        No company data matches the current filters.
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ProblemTable items={activeItems} />
    </div>
  );
}